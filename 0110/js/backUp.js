$(function () {
    $('body').load('01.html > .wrap', function () {
        pageMax = 6;
        linkIdx();
        pageBtn();
        setVideoPlay();
...
function setVideoPlay() {
    const $nextNotice = $('.next_notice'),
        ...
        _video = $videoWrap.find('video').get(0);
    _video.volume = 0.5;
...
function setNav() {
    ...
    $mainTit.find('>li').on('click', function () {
        subIdx = $(this).siblings().find('>li:nth-child(1)').attr('data-page');
        pageNum = subIdx < 10 ? '0' + subIdx : subIdx;
...
function linkIdx() {
    ...
    let trackSrc = $track.attr('src').split('.'),
        chVttFile = '.' + trackSrc[1].split('0')[0] + pageIdx + '.' + trackSrc[trackSrc.length - 1];
    $track.attr('src', chVttFile);

    videoSrc = $video.attr('src').split('/');
    videoNum = videoSrc[videoSrc.length -1];
    chNum = videoNum.split('.')[1]

    chVideoSrc = './'+videoSrc[1]+'/'+pageIdx+'.'+chNum;
    $video.attr('src',chVideoSrc);
