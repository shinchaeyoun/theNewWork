    ...
    $(window).on('scroll', function () {
      ...
      for(let i=0;i<$moviesSection.length; i++){
        if (window.scrollY > $movies - minusWin) {
          $moviesSection.eq(i).find('.sub_tit').addClass('active');
          $moviesSection.eq(i).find('.img_wrap').addClass('active');
          $moviesSection.eq(i).find('span').addClass('on');
          $moviesSection.eq(i).find('p').addClass('active');
        } else if (window.scrollY < $movies - minusWin) {
          $moviesSection.eq(i).find('.sub_tit').removeClass('active');
          $moviesSection.eq(i).find('.img_wrap').removeClass('active');
          $moviesSection.eq(i).find('span').removeClass('on');
          $moviesSection.eq(i).find('p').removeClass('active');
        };
      };
    });
  }).resize();
...
function flimFn(){
  ...
  const $fItem = $fcontainer.find('.slide_content');
  ...
  const $fImg = $fcontainer.children('.slide_content');

  let fslideIdx = $fItem.length / 5; // 1~4

  let fItemNum = $fItem.length;

  const fNextImg = function () {
    const imgSrc = $('#like .fliming .slide_content').eq(1).children('img').attr('src');
    const imgAlt = $('#like .fliming .slide_content').eq(1).children('img').attr('alt');
  ...
  const fPrevSlide = function () {
    ...
    $fcontainer.stop().animate({
      ...
      $('#like .fliming .slide .slide_container ul li').slice(fItemNum-1, fItemNum).prependTo($fcontainer);
      $fcontainer.css({
        left: -800
        // left: -$fItemWid + 'px'
      });
