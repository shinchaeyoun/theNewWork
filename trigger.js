...
window.addEventListener('DOMContentLoaded',function(){ //시작
    ...
    let hood = gsap.timeline({ // 원하는 트리거 이름 지정
        scrollTrigger: {
    ...
    hood.to(".hoody_item_1", { //클래스 이름을 지정해서 해당 클래스에 변경하고자 하는 값을 넣는다
        rotation:0,x:0 ,y:0,width:322, duration:1, scale:1.7
    },0); //순서 - 0,1,2,3 ... 순서로 작동 / 같은 숫자쓰면 동시 작동
    hood.to(".hoody_item_2", { //클래스 이름을 지정해서 해당 클래스에 변경하고자 하는 값을 넣는다
        rotation:0,x:0 ,y:0,width:322, duration:1, scale:1.7
    },0); //순서 - 0,1,2,3 ... 순서로 작동 / 같은 숫자쓰면 동시 작동
